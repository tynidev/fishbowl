...
Write-Host "  FISHBOWL API - CONFIGURATION TEST SUITE  " -ForegroundColor Cyan
Write-Host "=============================================" -ForegroundColor Cyan
...
function Add-TestResult {
    ...
    if ($Success) {
        ...
        if ($Details.Count -gt 0) {
            ...
            $Details.GetEnumerator() | ForEach-Object { Write-Host "      $($_.Name): $($_.Value | ConvertTo-Json -Compress -Depth 3)" -ForegroundColor Yellow }
        }
    }
...
Write-Host "`n--- SECTION: Game Creation Tests ---" -ForegroundColor Yellow

# Test Valid Game Creations
Write-Host "`n  Sub-Section: Valid Game Creations" -ForegroundColor Magenta
...
foreach ($config in $validGameCreations) {
    ...
    if ($response -and $response.gameCode) {
        Add-TestResult -TestName "$testName - Verification" -Success $true -Message "Game created with code: $($response.gameCode)."
        ...
        Add-TestResult -TestName "$testName - Verification" -Success $false -Message "Game creation call returned null unexpectedly without Invoke-ApiRequest logging failure for this test."
    } elseif ($response) {
         Add-TestResult -TestName "$testName - Verification" -Success $false -Message "GameCode missing in response." -Details @{ Response = $response }
    }
    # If $response is $null and Invoke-ApiRequest already logged a failure, we don't log again.
}

# Test Invalid Game Creations
Write-Host "`n  Sub-Section: Invalid Game Creations" -ForegroundColor Magenta
...
if ($createdGameCodes.Count -gt 0) {
    ...
    Write-Host "`n--- SECTION: Game Configuration Updates (Using Game: $gameCodeToUpdate) ---" -ForegroundColor Yellow
} else {
    Write-Host "`n--- SECTION: Game Configuration Updates (SKIPPED) ---" -ForegroundColor Yellow
    Write-Host "  No games were successfully created in the previous step. Skipping update tests." -ForegroundColor Yellow
...
if ($gameCodeToUpdate) {
    ...
    Write-Host "`n  Sub-Section: Valid Configuration Updates" -ForegroundColor Magenta
        ...
        if ($updateResponse) { 
            ...
            if ($verifyResponse) {
                ...
                if ($mismatchedFields.Count -eq 0) {
                    Add-TestResult -TestName "$testNamePrefix - Overall Verification" -Success $true -Message "All updated fields verified successfully."
                } else {
                    Add-TestResult -TestName "$testNamePrefix - Overall Verification" -Success $false -Message "Some fields did not match after update." -Details $mismatchedFields
                }
            }
        }
    }
    ...
    Write-Host "`n  Sub-Section: Invalid Configuration Updates" -ForegroundColor Magenta
...
Write-Host "`n--- SECTION: Team Change Edge Cases ---" -ForegroundColor Yellow
...
try {
    ...
    if (-not ($gameResponse -and $gameResponse.gameCode)) {
        Add-TestResult -TestName $createGameTestName -Success $false -Message "Failed to create game for edge case testing or gameCode missing." -Details @{ Response = $gameResponse }
        throw "Halting edge case tests: Prerequisite game creation failed."
    }
    $edgeCaseGameCode = $gameResponse.gameCode
    Add-TestResult -TestName "$createGameTestName - Verification" -Success $true -Message "Game $edgeCaseGameCode created."
    ...
    foreach ($playerName in $playerNames) {
        ...
        if (-not $joinResponse) { 
            ...
            Write-Host "    Skipping further player additions due to join failure for $playerName" -ForegroundColor Yellow
            # Potentially throw here if all players are critical, or just continue with fewer players
        } else {
            Add-TestResult -TestName "$joinTestName - Verification" -Success $true -Message "$playerName joined game $edgeCaseGameCode."
        }
    }
    ...
    if ($reduceResponse) {
        Add-TestResult -TestName "$reduceTeamTestName - Verification" -Success $true -Message "Team count reduction API call successful."
        ...
        if ($playersAfterUpdate -and $playersAfterUpdate.players) {
            ...
            if ($actualTeamCount -gt 0 -and $actualTeamCount -le 2) {
                 Add-TestResult -TestName "$verifyPlayersTestName - Team Count Verification" -Success $true -Message "Players correctly distributed into $actualTeamCount teams (expected <= 2). Distribution: $($teamCountsAfter | ConvertTo-Json -Compress -Depth 2)"
            } else {
                 Add-TestResult -TestName "$verifyPlayersTestName - Team Count Verification" -Success $false -Message "Players distributed into $actualTeamCount teams (expected <= 2). Distribution: $($teamCountsAfter | ConvertTo-Json -Compress -Depth 2)" -Details $teamCountsAfter
            }
        } elseif($playersAfterUpdate) {
            Add-TestResult -TestName "$verifyPlayersTestName - Player List" -Success $false -Message "Player list was empty or not in expected format after update." -Details @{ Response = $playersAfterUpdate }
        }
        # If $playersAfterUpdate is null, Invoke-ApiRequest already logged it.
    }
    Add-TestResult -TestName $edgeCaseSectionTestName -Success $true -Message "Team change edge case tests completed (individual results above)."

}
catch {
    # This catch is for the 'throw' or other unexpected script errors in this section
    Add-TestResult -TestName $edgeCaseSectionTestName -Success $false -Message "An error occurred: $($_.Exception.Message)"
    if ($edgeCaseGameCode) {
        Write-Host "  Note: Edge case test section failed for game $edgeCaseGameCode" -ForegroundColor Yellow
    }
...
Write-Host "Total Tests Attempted (approximate): $($Global:TestResults.Total)"
...
if ($Global:TestResults.Failed -eq 0) {
    if ($Global:TestResults.Skipped -eq 0) {
        ...
        Write-Host "ALL EXECUTED TESTS PASSED (some tests were skipped)." -ForegroundColor Yellow
    }
} else {
    Write-Host "SOME TESTS FAILED." -ForegroundColor Red
    Write-Host "Failed Test Details:" -ForegroundColor Yellow
    $Global:TestResults.Details | Where-Object { $_.Status -eq "Failed" } | ForEach-Object {
        ...
        if ($_.Details.Count -gt 0) {
            ...
            $_.Details.GetEnumerator() | ForEach-Object { Write-Host "      $($_.Name): $($_.Value | ConvertTo-Json -Compress -Depth 3)" -ForegroundColor Red }
        }
    }
}
