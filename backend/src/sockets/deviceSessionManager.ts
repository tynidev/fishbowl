...
export async function createOrUpdateDeviceSession(
    ...
    return await withTransaction(async (transaction) => {
        ...
        if (gameId) {
            const sessions = await select<DeviceSession>('device_sessions', {
                where: [
...
export async function getDeviceSession(
    ...
    return await withTransaction(async (transaction) => {
        const whereClause = [{ field: 'device_id', operator: '=' as const, value: deviceId }];

        if (gameId) {
            whereClause.push({ field: 'game_id', operator: '=' as const, value: gameId });
        }
 ...
 */
export async function getDeviceSessionBySocket(socketId: string): Promise<DeviceSession | null> {
    return await withTransaction(async (transaction) => {
        const sessions = await select<DeviceSession>('device_sessions', {
 ...
 */
export async function updateLastSeen(deviceId: string, gameId?: string): Promise<void> {
 ...
 */
export async function deactivateDeviceSessionBySocket(socketId: string): Promise<void> {
    await withTransaction(async (transaction) => {
        await update('device_sessions',
            {
                is_active: false,
 ...
 */
export async function getActiveSessionsForGame(gameId: string): Promise<DeviceSession[]> {
    return await withTransaction(async (transaction) => {
        return await select<DeviceSession>('device_sessions', {
            where: [
                { field: 'game_id', operator: '=', value: gameId },
...
export async function cleanupStaleSessions(): Promise<number> {
    return await withTransaction(async (transaction) => {
        ...
        const staleSessions = await select<DeviceSession>('device_sessions', {
            where: [
...
export async function removeOldSessions(): Promise<number> {
    return await withTransaction(async (transaction) => {
        ...
        const oldSessions = await select<DeviceSession>('device_sessions', {
            where: [
        ...
        await deleteRecords('device_sessions',
            [
                { field: 'last_seen', operator: '<', value: twentyFourHoursAgo },
                { field: 'is_active', operator: '=', value: false }
            ],
 ...
 */
export async function hasActiveSession(deviceId: string, gameId: string): Promise<boolean> {
    return await withTransaction(async (transaction) => {
        const sessions = await select<DeviceSession>('device_sessions', {
            where: [
                { field: 'device_id', operator: '=', value: deviceId },
 ...
 */
export async function getPlayerDeviceSessions(playerId: string): Promise<DeviceSession[]> {
    return await withTransaction(async (transaction) => {
        return await select<DeviceSession>('device_sessions', {
