    ...
    try {
        ...
        await withTransaction(async (transaction) => {
            ...
            if (!player || player.game_id !== gameCode) {
                ...
                socket.emit('error', { message: 'Player not found in this game. Make sure to join via REST API first.' });
                return;
            }
...
export async function handleAssignedTeam(
    ...
    try {
        ...
        await withTransaction(async (transaction) => {
            ...
            if(player.team_id !== teamId && player.team_id !== team.id && team.id !== teamId) {
                // If player is not assigned to the team then 
                // return error as only the REST API should handle team assignments
                socket.emit('error', { message: 'Player is not assigned to team. Use REST API to assign teams.' });
                return;
            }
