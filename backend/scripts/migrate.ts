...
async function runMigration() {
  try {
    ...
    for (const result of results) {
      if (result.success) {
        console.log(`  ✓ Applied migration ${result.version}: ${result.name} (${result.executionTime}ms)`);
      } else {
        console.error(`  ✗ Failed migration ${result.version}: ${result.name} - ${result.error}`);
      }
    }
